...
SOURCES (in priority order):
  ...
  3. football-data.org  — 12 competitions. Free tier, no key for basic data.
  4. ESPN hidden API    — Scores and fixtures, no auth required.
  5. API-Football free  — Some endpoints are unauthenticated for basic data.
  6. Sofascore public   — JSON API, no key needed for basic fixture data.
  7. ClubElo            — Elo ratings supplement (no key, free).
  8. FBref/StatHead     — Historical data via HTTP.
  9. World Football      — worldfootball.net scraper fallback.
  10. RapidAPI public   — Unauthenticated fixture data fallback.

DEDUPLICATION STRATEGY:
  canonical key = f"{league_id}:{home_team_id}:{away_team_id}:{date_ymd}"
  Within a league, a team can only appear once per day.
  Source priority: openligadb > thesportsdb > football_data > espn > sofascore > others
...
ESPN_LEAGUES = [
    ...
    {"slug": "kor.1",  "league_id": 292, "name": "K League 1",          "country": "South Korea"},
...
# ── SOURCE 4: football-data.org — free tier, no key for public data ───────────

# The free tier without a key only gives competition list + basic data.
# With the free API key (optional) it gives full fixtures and results.
# We use it here opportunistically — works better with the key set,
# but still tries without it.

FD_COMPS = [
    {"code": "BSA", "league_id": 71,  "name": "Brasileirao Serie A", "always": True},
    {"code": "CLI", "league_id": 13,  "name": "Copa Libertadores",   "always": True},
    {"code": "CSA", "league_id": 11,  "name": "Copa Sudamericana",   "always": True},
    {"code": "PL",  "league_id": 39,  "name": "Premier League",      "always": False},
    {"code": "BL1", "league_id": 78,  "name": "Bundesliga",          "always": False},
    {"code": "SA",  "league_id": 135, "name": "Serie A",             "always": False},
    {"code": "PD",  "league_id": 140, "name": "La Liga",             "always": False},
    {"code": "FL1", "league_id": 61,  "name": "Ligue 1",             "always": False},
    {"code": "DED", "league_id": 88,  "name": "Eredivisie",          "always": False},
    {"code": "PPL", "league_id": 94,  "name": "Primeira Liga",       "always": False},
    {"code": "ELC", "league_id": 40,  "name": "Championship",        "always": False},
    {"code": "CL",  "league_id": 2,   "name": "Champions League",    "always": False},
]

def source_football_data(match_history: dict, upcoming_data: dict,
                          dedup_keys: set, now_utc: datetime) -> dict:
    report = {"source": "football_data", "fixtures": 0, "history": 0,
               "leagues": [], "errors": [], "status": "ok"}

    fd_key = os.environ.get("FOOTBALL_DATA_API_KEY", "")
    headers = {"X-Auth-Token": fd_key} if fd_key else {}
    today = date.today()
    month = today.month

    active = [c for c in FD_COMPS if c["always"] or month >= 8 or month <= 5]
    if 6 <= month <= 7:
        active = FD_COMPS  # Try all in summer — costs nothing, returns 0 for off-season
    ...
    for comp in active:
        ...
        try:
            ...
            data = http_get(
                ...
                {"dateFrom": date_from, "dateTo": today.isoformat(), "status": "FINISHED"},
                headers=headers,
            )
            if data and "matches" in data:
                for m in data["matches"]:
                    ...
                    hteam_id = canonical_team_id(lid, hname)
                    ateam_id = canonical_team_id(lid, aname)
                    dt_str   = m.get("utcDate", "")
                    d_ymd    = parse_date_ymd(dt_str)
                    key      = canonical_fixture_key(lid, hteam_id, ateam_id, d_ymd)
                    if src_key not in match_history["matches"] and key not in dedup_keys:
                        match_history["matches"][src_key] = {
                            "api_fixture_id": src_key, "league_id": lid,
                            "league_name": comp["name"],
                            "home_team_id": hteam_id, "home_team_name": hname,
                            "away_team_id": ateam_id, "away_team_name": aname,
                            "match_date": dt_str, "season": CURRENT_SEASON,
                        ...
                        dedup_keys.add(key)
                        h += 1
            time.sleep(6)  # FD free: 10 req/min
            ...
            if data and "matches" in data:
                for m in data["matches"]:
                    ...
                    dt_str   = m.get("utcDate", "")
                    d_ymd    = parse_date_ymd(dt_str)
                    if not is_future_fixture(dt_str, now_utc): continue
                    key = canonical_fixture_key(lid, hteam_id, ateam_id, d_ymd)
                    if key not in dedup_keys:
                        upcoming_data["fixtures"].append({
                            "id": f"{lid}_{hteam_id}_{ateam_id}_{d_ymd}",
                            "api_fixture_id": src_key,
                            "fixture_date": dt_str,
            ...
            report["errors"].append(f"{comp['name']}: {e}")
            log.error(f"  FD {comp['name']} error: {e}")

        if f or h:
            report["leagues"].append(comp["name"])
            log.info(f"  football-data.org {comp['name']}: +{f} fixtures, +{h} history")
...
SOURCES = [
    ...
    ("football-data",  source_football_data,  "12 European + South American comps"),
