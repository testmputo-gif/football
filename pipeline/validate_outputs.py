...
def validate():
    ...
    if len(fx) > 0:
        ...
        # Warn if still dominated by old BSD data
        bsd_count = sources.get("bsd", 0)
        check(
            bsd_count < len(fx) * 0.8,
            "WARNING",
            f"{bsd_count}/{len(fx)} fixtures are from 'bsd' (old source) — "
            "FOOTBALL_API_KEY may not be set"
        )

        # Warn if very few leagues
        check(len(leagues) >= 3, "WARNING",
              f"Only {len(leagues)} league(s) in upcoming — expect 3+ with proper API keys")
