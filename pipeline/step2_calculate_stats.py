...
Computes per team:
  ...
  - Goals momentum (last 3 vs season)
...
from pipeline.data_store import (
    get_match_history, get_team_statistics, save_team_statistics,
...
# Exponential decay for form weighting
# Most recent match = weight 1.0, previous = 0.75, etc.
FORM_DECAY = 0.75
...
def run():
    ...
    # Sort matches by date (oldest first — important for Elo calculation)
    ...
    log.info(f"Step 2 complete in {elapsed}s: {len(team_stats)} teams, {len(league_stats)} leagues, {len(h2h_records)} H2H pairs")
...
def _calculate_league_baselines(matches: list) -> dict:
    """Compute per-league averages from all matches."""
    ...
    for m in matches:
        ...
        if hg > ag: league_data[lid]["results"].append("H")
        elif ag > hg: league_data[lid]["results"].append("A")
        else: league_data[lid]["results"].append("D")
    ...
    for lid, data in league_data.items():
        ...
        log.info(f"  League {lid}: {n} matches, avg goals {avg_home:.2f}/{avg_away:.2f}, HA={ha_factor:.3f}")
...
def _calculate_team_statistics(matches: list, league_stats: dict) -> dict:
    ...
    for team_id in all_team_ids:
        ...
        # ── Home stats ────────────────────────────────────────────────────────
        home_stats = _compute_split_stats(home_matches, is_home=True)
        away_stats = _compute_split_stats(away_matches, is_home=False)

        # ── Overall market rates ──────────────────────────────────────────────
        btts_count = sum(1 for m in all_matches
                        if (m.get("home_goals") or 0) > 0 and (m.get("away_goals") or 0) > 0)
        over15_count = sum(1 for m in all_matches
                          if (m.get("home_goals") or 0) + (m.get("away_goals") or 0) > 1.5)
        over25_count = sum(1 for m in all_matches
                          if (m.get("home_goals") or 0) + (m.get("away_goals") or 0) > 2.5)
        over35_count = sum(1 for m in all_matches
                          if (m.get("home_goals") or 0) + (m.get("away_goals") or 0) > 3.5)

        # ── Form string (last 5 results from team perspective) ────────────────
        form_string = _compute_form_string(all_matches[-5:], team_id)

        # ── Weighted form score (last 6, exponential decay) ───────────────────
        ...
        avg_goals_all = home_stats["avg_goals_scored"] * 0.5 + away_stats["avg_goals_scored"] * 0.5
        last_3 = all_matches[-3:]
        ...
        home_attack_str = home_stats["avg_goals_scored"] / la_home if la_home > 0 else 1.0
        home_defense_str = home_stats["avg_goals_conceded"] / la_away if la_away > 0 else 1.0
        away_attack_str = away_stats["avg_goals_scored"] / la_away if la_away > 0 else 1.0
        away_defense_str = away_stats["avg_goals_conceded"] / la_home if la_home > 0 else 1.0

        # Compute attack/defense ratings 0-100
        attack_rating = min(100, (home_attack_str + away_attack_str) / 2 * 50)
        defense_rating = min(100, max(0, 100 - (home_defense_str + away_defense_str) / 2 * 50))

        team_stats[team_id] = {
            "team_id": team_id,
            "league_id": lid,
            "total_matches_played": total,
            "data_sufficient": total >= 8 and home_stats["matches_played"] >= 4 and away_stats["matches_played"] >= 4,
            ...
            **{f"away_{k}": v for k, v in away_stats.items()},
            ...
            # Strength ratios
            "home_attack_strength": round(home_attack_str, 4),
            "home_defense_strength": round(home_defense_str, 4),
            "away_attack_strength": round(away_attack_str, 4),
            "away_defense_strength": round(away_defense_str, 4),
...
def _compute_form_string(matches: list, team_id: int) -> str:
    """Last 5 results as string: W=win, D=draw, L=loss."""
    result = ""
    for m in matches:
        hg = m.get("home_goals", 0) or 0
        ag = m.get("away_goals", 0) or 0
...
def _compute_weighted_form(matches: list, team_id: int, is_home: bool) -> float:
    """Exponentially weighted form score 0-1."""
    if not matches:
        return 0.5  # Neutral default
    total_weight = weights = 0.0
    for i, m in enumerate(reversed(matches)):
        ...
        weights += w
    return total_weight / weights if weights > 0 else 0.5
...
def _calculate_elo_ratings(matches: list) -> dict:
    """
    Calculate Elo ratings by replaying all historical matches in order.
    Returns {team_id: {elo_rating, elo_matches_played}}
    """
    ...
    for m in matches:
        ...
        ra = ratings[htid]
        rb = ratings[atid]
        # Home advantage built into Elo: add 50 points to home team expected
    ...
    return {
        tid: {"elo_rating": round(ratings[tid], 2), "elo_matches_played": played[tid]}
        for tid in ratings
    }


def _calculate_h2h(matches: list) -> dict:
    """Build H2H records for all team pairs."""
    h2h = defaultdict(lambda: {
        "matches": [], "home_wins": 0, "away_wins": 0, "draws": 0,
    ...
    for m in matches:
        ...
        # Store perspective from smaller_id team's view
        if htid < atid:
            if hg > ag: record["home_wins"] += 1
            elif ag > hg: record["away_wins"] += 1
            else: record["draws"] += 1
        else:
            if ag > hg: record["home_wins"] += 1
            elif hg > ag: record["away_wins"] += 1
            else: record["draws"] += 1
        ...
        ac = m.get("away_corners")
        if hc is not None and ac is not None:
            record["total_corners"] += hc + ac
            record["corner_matches"] += 1

        hy = m.get("home_yellow_cards") or 0
        ay = m.get("away_yellow_cards") or 0
        hr = m.get("home_red_cards") or 0
        ar = m.get("away_red_cards") or 0
        record["total_cards"] += hy + ay + hr + ar
        record["matches"].append(m.get("api_fixture_id"))

    result = {}
    for key, rec in h2h.items():
        n = rec["home_wins"] + rec["away_wins"] + rec["draws"]
        if n < 2:
            continue
        result[key] = {
            "matches_played": n,
