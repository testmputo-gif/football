...
Computes per team:
  ...
  - Elo ratings (updated after each result, time-decayed)
  - Goals momentum (last 3 vs season average)
  - Rest days between matches
  - Referee statistics (built from match history)

Computes per league:
  - Average goals home/away
  - Home advantage factor
  - Win/draw/loss rates
  - Average corners and cards per game
...
# Time-decay half-life: a match 90 days ago counts half as much as today
TIME_DECAY_HALF_LIFE_DAYS = 90


def _decay_weight(match_date_str: str, now: datetime = None) -> float:
    """
    Exponential time-decay weight.
    A match played TIME_DECAY_HALF_LIFE_DAYS ago gets weight 0.5.
    ...
    try:
        ...
        return 0.5 ** (days_ago / TIME_DECAY_HALF_LIFE_DAYS)
    except Exception:
        return 1.0
...
def run():
    ...
    # Sort by date oldest-first — critical for Elo sequential update
    ...
    team_stats = _calculate_team_statistics(matches, league_stats, now_utc)

    # Step 2c: Elo ratings — calculate then merge cleanly into team_stats
    ...
    for team_id, elo_data in elo_ratings.items():
        ...
        if tid in team_stats:
            ...
            # Team appeared in elo but was filtered out of team_stats
            # (e.g. fewer than 3 total matches) — still store elo data
            team_stats[tid] = {
                "team_id":              team_id,
                "league_id":            None,
                "total_matches_played": elo_data.get("elo_matches_played", 0),
                "data_sufficient":      False,
                **elo_data,
            }
            elo_new += 1
    log.info(f"Elo: {elo_merged} merged, {elo_new} new entries")

    # Step 2d: H2H records
    h2h_records = _calculate_h2h(matches)

    # Step 2e: Referee statistics (built from match history — no extra API call)
    referee_stats = _calculate_referee_stats(matches)
    ...
    save_referee_data(    {"referees": referee_stats})
...
def _calculate_team_statistics(matches: list, league_stats: dict, now_utc: datetime) -> dict:
    """Compute per-team rolling statistics with time-decay weighting."""
    ...
    for team_id in all_team_ids:
        ...
        # Goals momentum: time-decayed recent scoring vs season average
        avg_goals_all = (home_s["avg_goals_scored"] + away_s["avg_goals_scored"]) / 2
        last_3        = all_m[-3:]
        ...
        # Rest days (days since previous match, capped at 30)
        rest_days = _compute_rest_days(all_m)
        ...
        team_stats[str(team_id)] = {
            ...
            **{f"away_{k}": v for k, v in away_s.items()},
            ...
            # Elo — populated by _calculate_elo_ratings + merge below
...
def _compute_rest_days(matches: list) -> int:
    """Days since the team's most recent match (max 30, default 7)."""
    if len(matches) < 2:
        return 7
    try:
        last_date = datetime.fromisoformat(
            matches[-1].get("match_date", "").replace("Z", "+00:00")
        )
...
def _compute_form_string(matches: list, team_id) -> str:
    result = ""
    for m in matches:
        hg      = m.get("home_goals", 0) or 0
        ag      = m.get("away_goals", 0) or 0
...
def _compute_weighted_form(matches: list, team_id, is_home: bool) -> float:
    if not matches:
        return 0.5
...
def _team_goals_in_matches(matches: list, team_id) -> float:
    total = 0
    for m in matches:
        is_home = m.get("home_team_id") == team_id
...
def _calculate_elo_ratings(matches: list) -> dict:
    """
    Sequential Elo update through all matches (oldest first).
    K-factor reduces from 32 to 20 after 30 games (settled team effect).
    Home advantage: home team gets +50 Elo points in expected score calc.
    """
    ...
    for m in matches:
        ...
        # Home advantage baked into expected score (+50 Elo equivalent)
        ...
        # Reduce K after 30 games — stabilises established team ratings
        k_home = 20 if played[htid] > 30 else ELO_K
        k_away = 20 if played[atid] > 30 else ELO_K

        ratings[htid] = ra + k_home * (sa - ea)
        ratings[atid] = rb + k_away * (sb - eb)
        played[htid] += 1
        played[atid] += 1
...
def _calculate_referee_stats(matches: list) -> dict:
    """
    Build referee profiles from match history — zero extra API calls.
    Accumulates yellow cards, red cards, total matches officiated.
    Only saves referees with >= 3 matches.
    """
    ...
    for m in matches:
        ...
        hr = m.get("home_red_cards")    or 0
        ar = m.get("away_red_cards")    or 0
        ref_data[ref]["total_yellows"] += hy + ay
        ref_data[ref]["total_reds"]    += hr + ar
    ...
    for name, d in ref_data.items():
        ...
        referees[name] = {
            ...
            "avg_yellow_cards":    round(d["total_yellows"] / n, 3),
            "avg_red_cards":       round(d["total_reds"]    / n, 3),
        }

    log.info(f"  Built profiles for {len(referees)} referees ({len(ref_data)} total seen)")
    return referees
