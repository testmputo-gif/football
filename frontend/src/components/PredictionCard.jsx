...
export default function PredictionCard({ fixture }) {
  ...
  const hasAnyConf = hasPrediction && (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-400 font-medium truncate max-w-[150px]">{fixture.league_name}</span>
        </div>
        ...
        {hasPrediction && hasAnyConf ? (
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-slate-500 flex items-center gap-1">
              <AlertCircle size={10} />Building data ({fixture.home_matches_used ?? 0}/8)
            </span>
          ...
          <div className="text-center text-xs text-slate-600 flex items-center justify-center gap-1">
            <AlertCircle size={10} />More data needed
          </div>
        )}
      </div>
      <div className="mt-2 text-xs text-slate-600 group-hover:text-emerald-500 text-center transition-colors">
        Full analysis →
      </div>
    </Link>
  )
