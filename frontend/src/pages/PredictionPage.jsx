...
// Safe venue string - handles dict or string
function venueStr(venue) {
  if (!venue) return null
  if (typeof venue === 'string') return venue
  if (typeof venue === 'object') return venue.name || null
  return null
}
...
export default function PredictionPage() {
  ...
  if (notFound || !fixture) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-4">
        ...
        <p className="text-slate-400 text-sm">This fixture may have expired from the 30-day window.</p>
        <Link to="/fixtures" className="inline-block text-sm text-emerald-400 hover:underline">← Back to fixtures</Link>
      </div>
    )
  }
  ...
  const venue    = venueStr(fixture.venue)
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-5 text-xs text-slate-500">
          <div className="flex items-center gap-1.5">
            ...
            {fixture.league_country && <span className="text-slate-600">· {fixture.league_country}</span>}
          </div>
          <span>{dateStr}</span>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 mb-5">
          <div className="flex-1 flex flex-col items-center gap-2 text-center">
            ...
            {fixture.home_elo && <span className="text-xs text-slate-500">Elo {Math.round(fixture.home_elo)}</span>}
          </div>
          ...
          <div className="text-center px-2 shrink-0">
            ...
            {venue && <div className="text-xs text-slate-600 mt-2 max-w-[100px] mx-auto leading-tight">{venue}</div>}
          </div>
          ...
          <div className="flex-1 flex flex-col items-center gap-2 text-center">
            ...
            {fixture.away_elo && <span className="text-xs text-slate-500">Elo {Math.round(fixture.away_elo)}</span>}
          </div>
        </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
        {activeTab === 'predictions' && (
          fixture.no_prediction_reason
            ? <p className="text-slate-500 text-sm text-center py-4">No predictions — see notice above.</p>
            : <div>
                ...
                <div className="mt-3 pt-3 border-t border-slate-700/60 text-xs text-slate-500 flex justify-between">
                  <span>Engine: {fixture.ml_used ? 'ML ensemble' : 'Dixon-Coles'}</span>
        ...
        {activeTab === 'stats' && (
          <div className="space-y-1">
            ...
            {fixture.home_elo && <StatBar label="Elo rating" homeVal={fixture.home_elo} awayVal={fixture.away_elo} />}
        ...
        {activeTab === 'matrix' && (
          fixture.score_matrix
            ...
            : <p className="text-slate-500 text-sm text-center py-4">Score matrix not available.</p>
        )}

        {activeTab === 'reasoning' && (
          fixture.reasoning && Object.keys(fixture.reasoning).length > 0
            ? <div className="space-y-4">
                {Object.entries(fixture.reasoning).map(([market, text]) => (
                  <div key={market} className="border-b border-slate-700/60 pb-4 last:border-0 last:pb-0">
                    ...
                    <p className="text-slate-300 text-sm leading-relaxed">{String(text)}</p>
                  </div>
                ))}
              </div>
            : <p className="text-slate-500 text-sm text-center py-4">Reasoning not available.</p>
        )}
      </div>
    </div>
  )
