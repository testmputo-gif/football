...
// Fixed: handles all fixture ID formats including BSD prefixed IDs
...
async function findFixture(targetId) {
  ...
  // Decode URL encoding (handles %5F etc)
  ...
  // Check last 7 days of prediction files
  const today = new Date()
  for (let i = 0; i < 7; i++) {
    const d = new Date(today)
    d.setDate(today.getDate() - i)
    const dateStr = d.toISOString().split('T')[0]
    const url = i === 0 ? '/predictions/latest.json' : `/predictions/${dateStr}.json`
    try {
...
export default function PredictionPage() {
  ...
  useEffect(() => {
    ...
    setNotFound(false)
    findFixture(id)
      .then(f => { if (f) setFixture(f); else setNotFound(true) })
  ...
  if (notFound)  return (
    <div className="text-center py-24 space-y-3">
      ...
      <p className="text-slate-600 text-xs font-mono break-all max-w-md mx-auto">{decodeURIComponent(id)}</p>
      <Link to="/fixtures" className="inline-block text-emerald-400 hover:underline text-sm">
        ← Back to fixtures
      </Link>
    </div>
  )
  ...
  let formattedDate  = '—'
  let generatedDate  = '—'
  try { if (fixture.fixture_date) formattedDate = format(new Date(fixture.fixture_date), 'EEE d MMM · HH:mm') } catch (e) {}
  try { if (fixture.generated_at)  generatedDate  = format(new Date(fixture.generated_at),  'dd MMM HH:mm') }    catch (e) {}
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          {fixture.no_prediction_reason ? (
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 flex gap-2">
              ...
              <div>
                <p className="text-yellow-300 text-sm font-medium">No prediction available yet</p>
                <p className="text-yellow-200/70 text-xs mt-1">{fixture.no_prediction_reason}</p>
                <p className="text-slate-500 text-xs mt-2">
                  Improves automatically as more match data accumulates.
                </p>
              </div>
            </div>
          ) : (
          ...
          <p className="text-xs text-slate-600 mt-4 pt-3 border-t border-slate-700/40">
            ...
            {fixture.ml_used ? ' · ML model active' : ' · Dixon-Coles model'}
            {fixture.data_source ? ` · Source: ${fixture.data_source}` : ''}
          </p>
        </div>
      )}
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-2">
          ...
          {fixture.referee_name && (
            <div className="pt-3 border-t border-slate-700">
              ...
              <p className="text-white text-sm">{fixture.referee_name}</p>
            </div>
          )}
          <div className="pt-3 border-t border-slate-700/40 space-y-1">
            <p className="text-xs text-slate-500">League: {fixture.league_name} ({fixture.league_country})</p>
            <p className="text-xs text-slate-500">Season: {fixture.season}</p>
            <p className="text-xs text-slate-500">Data: {fixture.data_source || 'football-data.org'}</p>
          </div>
        </div>
      )}
      ...
      {tab === 'matrix' && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <p className="text-xs text-slate-500 mb-4">
            Each cell = probability of that exact score. Yellow ring = most likely.
          </p>
      ...
      {tab === 'reasoning' && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-4">
          ...
          {Object.keys(reasoning).length === 0 ? (
            <p className="text-slate-500 text-sm">No reasoning available for this fixture.</p>
          ) : (
